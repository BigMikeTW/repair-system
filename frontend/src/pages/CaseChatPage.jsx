// CaseChatPage — 案件溝通（對工程師內部對話）
// P4-1：與 ChatPage（線上客服/對業主）完全分離的獨立頁面
...
export default function CaseChatPage() {
  ...
  const { data: conversations } = useQuery('conversations_internal', () =>
    chatAPI.getConversations().then(r => r.data),
    { refetchInterval: 15000 }
  );

  const { data: fetchedMessages } = useQuery(
    ['messages_internal', activeCaseId],
    () => activeCaseId ? chatAPI.getMessages(activeCaseId).then(r => r.data) : Promise.resolve([]),
    { enabled: !!activeCaseId }
  );

  useEffect(() => { if (fetchedMessages) setMessages(fetchedMessages); }, [fetchedMessages]);
  ...
  useEffect(() => {
    ...
    socket.on('new_message', (msg) => {
      ...
      qc.invalidateQueries('conversations_internal');
    });
    chatAPI.markRead(activeCaseId).catch(() => {});
    return () => { socket.emit('leave_case', activeCaseId); socket.off('new_message'); };
  }, [socket, activeCaseId]);

  useEffect(() => { bottomRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [messages]);

  const selectConversation = (id) => { setActiveCaseId(id); setMessages([]); };
  ...
  const sendMessage = async () => {
    ...
    try {
      if (socket?.connected) {
        socket.emit('send_message', { caseId: activeCaseId, message: text, senderName: user.name, senderRole: user.role });
      } else {
    ...
    <div className="flex h-[calc(100vh-48px)] overflow-hidden">
      ...
      <div className="w-64 bg-white border-r border-gray-100 flex flex-col flex-shrink-0">
        <div className="px-4 py-3 border-b border-gray-100">
          <h2 className="text-sm font-medium text-gray-900">案件溝通</h2>
          <p className="text-xs text-gray-400 mt-0.5">與工程師溝通派工事項</p>
        </div>
            ...
            >
              <div className="flex items-center justify-between mb-0.5">
                ...
                {parseInt(conv.unread_count) > 0 && (
                  <span className="w-4 h-4 text-xs bg-danger text-white rounded-full flex items-center justify-center flex-shrink-0 ml-1">{conv.unread_count}</span>
                )}
              </div>
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                {conv.case_type && <><span className="text-gray-300">·</span><span className="text-xs text-gray-400 truncate">{conv.case_type}</span></>}
              </div>
            </button>
          ))}
          {!conversations?.length && (
            <div className="py-10 text-center text-xs text-gray-400">
              <MessageSquare size={24} className="mx-auto mb-2 text-gray-200" />沒有對話記錄
            </div>
          )}
        </div>
      </div>
        ...
        {activeCaseId && activeConv ? (
          <>
            <div className="px-5 py-3 bg-white border-b border-gray-100 flex items-center justify-between">
              ...
              <span className="badge badge-teal text-xs">工程師頻道</span>
            </div>
                ...
                return (
                  <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[70%] ${isMe ? 'items-end' : 'items-start'} flex flex-col`}>
                      ...
                      <div className={isMe ? 'msg-bubble-user' : 'msg-bubble-agent'}>{msg.message}</div>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="flex-1 flex items-center justify-center bg-gray-50">
            <div className="text-center">
              ...
              <div className="text-sm text-gray-400">選擇左側對話，開始工程師溝通</div>
            </div>
          </div>
        )}
      </div>
